...
const MyTableBazargah = ({ columns, data ,getData,bulkJob ,  rowProps = (row) => ({}) }) => {
    ...
    //     $('#reportTd th').each(function(idx, el) {
    //         var check = !!$('#reportTd tbody tr').find('td:eq(' + idx + ')').filter(function() {
    //           return $.trim($(this).html()).length;
    //         }).length;
           
              
    //           $('#reportTd tr').find('td:eq(' + idx + '), th:eq(' + idx + ')').show();
    //           if (!check) {
    //               $('#reportTd tr').find('td:eq(' + idx + '), th:eq(' + idx + ')').hide();
    //           }
    ...
    return (
        <Fragment>
            ...
            >
                ...
                <table id='reportTd' className='table responsive mb-4 ' {...getTableProps()}
