...
const CustomerAttachment: React.FC<Props> = ({ order, params, modalIsOpenUpload, closeModalForUpload, setIsOpenUpload }) => {
        ...
        <div className=" mb-1    rounded ">
          ...
          {show === true && loading ? (
            ...
            <div className="info-Item">
              ...
              <div className="form-group mb-4 textOnInput col-lg-12 col-xs-12    border-dark  mt-4 p-2">
                <div className="text-center">
                  <div className=" row text-center">
                    {newAttachment.map((item: any) =>
                      <div key={item.id} onClick={()=>item.path.split('/')[3].split('.')[1] === 'pdf'?()=>false: handelPreview(item)}  className={item.attachmentTypeId === 2 ? " img col-md-2 col-sm-12 " : "  col-md-2 col-sm-12 "} >
                        {item.path.split('/')[3].split('.')[1] === 'pdf' ?  <a onClick={()=>window.open(`${attachmet}${item.path}`)}><ImFilePdf size={'10rem'} title={item.name}/></a>:<img src={`${attachmet}${item.path}`} className=" img " alt={item.name} />}
                        {item.path.split('/')[3].split('.')[1] !== 'pdf'?
                        (item.deleted===false?
                          ...
                          {item.attachmentTypeId === 2 ?
                            ...
                            : item.attachmentTypeId === 1 ?
                              ...
                              : item.attachmentTypeId === 0 ?
                                ...
                                ''}
                        </div>:<div className="detial-img-deleted">
